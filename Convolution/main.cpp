...
int main(int argc, char * argv[])
{
    ...
    // Load kernel
    const char * kernelFile = "convolutionFilter.cl";
    ...
    int filter_size = 3; // example of a convolution with size of 3*3
    const float average_mask[filter_size*filter_size] = {1, 1, 1,
                                                         1, 1, 1,
                                                         1, 1, 1};
    cl_mem maskbuffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, filter_size*filter_size*sizeof(float),(void *)&average_mask, &status);
    if(status != CL_SUCCESS){
